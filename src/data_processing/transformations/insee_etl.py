...
def transform(
    ...
    """
    # 1. Extract and explode communes referential
    communes_df = (
        df.filter(F.col("communes").isNotNull())
        .select(F.explode("communes").alias("c"))
        .select(
            F.col("c.code").alias("code_insee"),
            F.col("c.nom").alias("nom_commune"),
            F.col("c.codeDepartement").alias("code_departement"),
            F.col("c.codeRegion").alias("code_region"),
        )
        .dropDuplicates(["code_insee"])
    )

    # Normalize commune code to 5 chars (e.g. Corsica, DOM, etc.)
    communes_df = communes_df.withColumn("code_insee", F.lpad(F.col("code_insee"), 5, "0"))

    # Fallback if no communes metadata is found in S3 files (e.g. all files are legacy)
    if communes_df.isEmpty():
        print("[silver-insee] Communes referential is empty in Bronze files. Fetching dynamically from geo.api.gouv.fr...")
        import urllib.request
        import json
        url = "https://geo.api.gouv.fr/communes?fields=code,nom,codeDepartement,codeRegion"
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0"})
            with urllib.request.urlopen(req, timeout=30) as response:
                data = json.loads(response.read().decode("utf-8"))
            
            # Create a Spark DataFrame from the API response
            communes_df = spark.createDataFrame(data, schema=COMMUNE_SCHEMA).select(
            ...
            communes_df = communes_df.withColumn("code_insee", F.lpad(F.col("code_insee"), 5, "0"))
            print(f"[silver-insee] Successfully loaded {communes_df.count()} communes from geo.api.gouv.fr")
        except Exception as e:
            print(f"[silver-insee] Failed to fetch communes from geo.api.gouv.fr: {str(e)}")
            raise
    ...
    obs_df = (
        ...
        .filter(
            (F.col("sex") == "_T")
            & (F.col("age") == "_T")
            & F.col("geo_code").contains("COM-")
        )
        # GEO = "<year>-COM-<insee>" -> strip everything up to and incl. "COM-".
        .withColumn("code_insee", F.lpad(F.regexp_replace("geo_code", "^.*COM-", ""), 5, "0"))
