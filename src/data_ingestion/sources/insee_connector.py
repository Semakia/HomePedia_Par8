...
class InseeConnector:
    ...
    def _fetch_melodi_batch(self, codes: list[str], year: int) -> list[dict]:
        """
        Query INSEE Melodi API for a batch of commune codes.
        """
    ...
    def ingest(self, year: int, departement: str, overwrite: bool = False) -> dict:
        ...
        # 1. Fetch communes metadata from department
        try:
        ...
        # 2. Fetch observations from Melodi in throttled batches
        all_observations = []
        batches = [commune_codes[i:i + BATCH_SIZE] for i in range(0, len(commune_codes), BATCH_SIZE)]
        ...
        for idx, batch in enumerate(batches):
            ...
            try:
                observations = self._fetch_melodi_batch(batch, year)
                all_observations.extend(observations)
            except Exception as e:
                log.error("insee.melodi.batch.failed", batch_num=idx+1, error=str(e))
                raise
