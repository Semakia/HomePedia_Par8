...
class FilosofiConnector:
    ...
    def _fetch_melodi_batch(self, codes: list[str], year: int) -> list[dict]:
        """Query the FiLoSoFi cube for a batch of commune codes."""
    ...
    def ingest(self, year: int, departement: str, overwrite: bool = False) -> dict:
        ...
        batches = [
            commune_codes[i:i + BATCH_SIZE]
            for i in range(0, len(commune_codes), BATCH_SIZE)
        ]
        for idx, batch in enumerate(batches):
            ...
            try:
                all_observations.extend(self._fetch_melodi_batch(batch, year))
            except Exception as e:
                log.error("filosofi.melodi.batch.failed", batch_num=idx + 1, error=str(e))
                raise
