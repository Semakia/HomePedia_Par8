...
class QualityMonitor:
    ...
    def _check_range(
        ...
        # ignore_nulls: only check present values (for legitimately nullable cols)
        cond = F.lit(False) if rule.get("ignore_nulls") else F.col(col).isNull()
        if lo is not None:
    ...
    def _check_set(
        ...
        bad = ~F.col(col).isin(allowed)
        cond = bad if rule.get("ignore_nulls") else (F.col(col).isNull() | bad)
        viol = df.filter(cond).count()
        pct = self._pct(viol, total)
        thr = rule.get("max_violation_pct", 0.0)
        return CheckResult(
